...
.do_player_turn
	...
	call BattleRandom
;	cp 10 percent
	cp 20 percent ; chance to defrost
	ret nc
	xor a
...
;	ld a, [wBattleType]        ;silvermirror enable loss text
;	cp BATTLETYPE_CANLOSE      ;
;	jr nz, .not_canlose
	ld a, [wBattleMode]
	dec a ; wild?
	...
	ld h, [hl]
	or h                       ;
	jr z, .no_loss_text        ;silvermirror enable loss text
...
;.not_canlose
.no_loss_text                  ;silvermirror enable loss text
	ld a, [wLinkMode]
	and a
...
;	call BadgeStatBoosts ;silvermirror- remove badge stat boosts
	ret
...
BattleMenu_PKMN:
; is actually "BAIT" in safari game
	ld a, [wBattleType]
	cp BATTLETYPE_SAFARI
...
;	callfar BadgeStatBoosts ;silvermirror- remove badge stat boosts
