	db PORYGON ; 137

;	db  65,  60,  70,  40,  85,  75  ;PORYGON
	db  85,  80,  90,  60, 105,  95  ;PORYGON2
	;   hp  atk  def  spd  sat  sdf

	db NORMAL, NORMAL ; type
	db 45 ; catch rate
	db 130 ; base exp
	db NO_ITEM, NO_ITEM ; items
	db GENDER_UNKNOWN ; gender ratio
;	db 100 ; unknown 1
	db 10 ;20 ; step cycles to hatch
;	db 5 ; unknown 2
	INCBIN "gfx/pokemon/porygon/front.dimensions"
;	dw NULL, NULL ; unused (beta front/back pics)
	db GROWTH_MEDIUM_FAST ; growth rate
	dn EGG_MINERAL, EGG_MINERAL ; egg groups

	; tm/hm learnset
	tmhm AERIAL_ACE, HONE_CLAWS, GIGA_IMPACT, TOXIC, DOUBLE_EDGE, ICE_BEAM, BLIZZARD, HYPER_BEAM, THUNDERBOLT, THUNDER, PSYCHIC_M, DOUBLE_TEAM, REFLECT, BIDE, SWIFT, DREAM_EATER, REST, THUNDER_WAVE, TRI_ATTACK, SUBSTITUTE, FALSE_SWIPE, CURSE, SPARK, HIDDEN_POWER, SUNNY_DAY, SNORE, ICY_WIND, PROTECT, RAIN_DANCE, ENDURE, IRON_TAIL, SWAGGER, SLEEP_TALK, THIEF, THUNDERBOLT, ICE_BEAM
;tmhm CURSE, TOXIC, SPARK, HIDDEN_POWER, SUNNY_DAY, SNORE, BLIZZARD, HYPER_BEAM, ICY_WIND, PROTECT, RAIN_DANCE, ENDURE, IRON_TAIL, THUNDER, PSYCHIC_M, DOUBLE_TEAM, SWAGGER, SLEEP_TALK, SWIFT, DREAM_EATER, REST, THIEF, NIGHTMARE, FLASH, THUNDERBOLT, ICE_BEAM
	; end
;SKULL_BASH
;PSYWAVE
;MIMIC
;NIGHTMARE
;FLASH

;	db  85,  80,  90,  60, 105,  95  ;PORYGON2
	;   hp  atk  def  spd  sat  sdf
